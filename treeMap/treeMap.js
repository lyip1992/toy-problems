...
Tree.prototype.map = function (callback) {
  ...
  var recurse = function(callback, newNode, oldNode) {
    
    if( oldNode.value ) newNode.value = callback(oldNode.value);

    for( var i = 0; i < oldNode.children.length; i++ )
      if( !newNode.children[i] ) newNode.children[i] = new Tree();
      recurse( callback, newNode.children[i], oldNode.children[i] );

  };

  recurse(callback, newTree, this);
...
Tree.prototype.mapInPlace = function (callback) {
  ...
  if( this.value ) this.value = callback(this.value);
